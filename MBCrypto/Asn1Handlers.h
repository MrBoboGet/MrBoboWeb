...
enum class ASN1PrimitiveTagNumber : uint8_t
{
    ...
    IA5String = 22,
    UTCTime = 23,
    NonStandardTag = uint8_t(255)>>3//00011111 alla dem sista �r ettor
};
...
public:
    ...
    ASN1TagValue ExtractTagData()
    {
        ...
        // om tagtypen inte �r nonstandard slutar vi h�r
        ...
        {
            //vill ej deala med det h�r just nu, s� vi bara assertar false om det h�nder och blir ledsen
            assert(false);
        }
        return(ReturnValue);
    }
    uint64_t ExtractLengthOfType()
    {
        ...
        Offset += 1;
        //kollar huruvida det �r short eller l�ng form
        if (FirstLengthByte>>7 == 0)
        {
            //short form, resten av bitarna �r det som faktiskt avg�r v�rdet
            ReturnValue = FirstLengthByte;//tar bort den st�srta ibten
        }
        else
        {
            //f�rsta bitten avg�r hur m�nga bits som f�ljer 
            uint8_t NumberOfLengthBits = FirstLengthByte - 128;
            //tar inte i �tanke att den kan overflowa
            for (int i = NumberOfLengthBits-1; i >= 0; i--)
            {
                //big endian, s� den f�rsta biten �r den st�rsta
