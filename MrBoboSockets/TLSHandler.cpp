...
namespace TLS1_2
{
	...
	int SizeOfProccessedHelloClientStruct(TLS1_2HelloClientStruct& StructToProcess)
	{
		...
		//eftersom varje extension nu sedan kan innah�lla varierande m�ngd data beh�ver vi loopa igen alla
	...
	void FillArrayWithHelloClientData(uint8_t* ArrayToFill, TLS1_2HelloClientStruct& Data)
	{
		...
		//extensiones s�ger inte sin l�ngd innan om dem om inte finns
	...
	TLSServerPublickeyInfo GetServerPublicKey(std::string& ServerCertificateData)
	{
		...
		*/
		...
		//f�rst entrar vi tbscertificate f�ltet
		Parser.ExtractTagData();
		Parser.ExtractLengthOfType();

		//eftersom att den har ett default value kollar vi f�rst och fr�mst om den finns, g�r den inte det s� vet vi att vi skippar r�tt grej i alla fall
		ASN1TagValue VersionTag = Parser.ExtractTagData();
		if (VersionTag.AlternateTagValue == 0)
		{
			//det �r faktiskt en tag vi extractar och inte serialnumber

			//vi skippar helt enkelt typen vi b�rjar extracta, som vi nu vet �r serialnumber
		...
		//nu vill vi entra subjectpublickeyinfo f�ltet
		...
		//nu j�vlar, nu �r vi �ntligen i sj�lva RSA keyn som vi vill ha
		...
		//Relevant om den f�rsta certifikaten inte ineh�ller public keyn
		...
		//nu koller vi l�ngden till n�sta value
		return(ReturnValue);
	}
...
RSAPublicKey TLSHandler::ExtractRSAPublicKeyFromBitString(std::string& BitString)
{
	...
	Parser.ExtractByte();
	Parser.ExtractTagData();
	Parser.ExtractLengthOfType();//vi vill skippa den del som bara encodar v�r dubbel int typ
	Parser.ExtractTagData();
...
std::string TLSHandler::I2OSP(MrBigInt NumberToConvert, uint64_t LengthOfString)
{
	...
	{
		//returnar en string som sett att grejen �r big endian, vi appendar dem sista grejerna och till slut s� reversar vi den
...
{
	//vi utg�r alltid ifr�n att det �r rsa med specifika algoritmer
	...
	//OBSS!"!"!"!"!"!!  dem f�rsta bytesen �r variationen vi ville b�rjade att skicka, dvs jag hard codar 3 3
	PreMasterSecret[0] = 3;
	PreMasterSecret[1] = 3;
...
{
	//vi b�rjar med att skicka ett client hello
	...
	//passar �ven p� att fylla v�ra parameters med v�ran random data
	for (int i = 0; i < 32; i++)
	{
	...
	//nu har vi initialisat allt data vi beh�ver f�r hellosend grejen, nu s� ska vi skicka detta record till servern
...
void TLSHandler::SendChangeCipherMessage(MBSockets::ConnectSocket* SocketToConnect)
{
	...
	{
		//egentligen lite os�ker p� denna
...
void TLSHandler::p_EstablishPreMasterSecret(TLS1_2::SecurityParameters& ConnectionParams, std::vector<std::string> const& ServerHelloResponse, MBSockets::ConnectSocket* SocketTouse)
{
	...
	std::string ServerCertificate = ServerCertificateMessage.substr(5 + 4 + 3 + 3/*f�rsta trean �r eftersom den b�rjhar med en lengthy byte av alla data, andra �r f��r l�ngen av f�rsta certfiikaten*/);
...
MBError TLSHandler::InitiateHandShake(MBSockets::ConnectSocket* SocketToConnect,std::string const& HostName)
{
	...
	std::string ServerHelloResponseDataFragment = ServerHelloResponseData[0].substr(5/*l�ngd av record layern*/+4/*l�ngd av handshake grejen innan datan*/);
...
MBError TLSHandler::EstablishHostTLSConnection(MBSockets::ConnectSocket* SocketToConnect)
{
	...
	//om clienten skicker ett session id s� vill vi ta och anv�nda den sesssion id:n v�rden
	...
	//f�r s� tar vi och decrypter premasterkeyn fr�n dens f�rsta svar
	//debug grejer
...
TLS1_2::TLS1_2HelloClientStruct TLSHandler::ParseClientHelloStruct(std::string const& ClientHelloData)
{
	...
	//vi b�rjar med att ta bort det som �r p� toppen, dvs record och handshake layer headern,total 5+4 = 9
	Parser.Extract32();
	Parser.Extract32();
	Parser.Extract8();
...
MBError TLSHandler::ResumeSession(std::string const& SessionID,TLS1_2::TLS1_2HelloClientStruct const& ClientHello ,MBSockets::ConnectSocket* SocketToConnect)
{
	...
	GenerateKeys();
	SendChangeCipherMessage(SocketToConnect);
	//�r ju fr�gan om det ska vara 0 eller 1 recored sent?
	std::string VerifyDataMessage = GenerateVerifyDataMessage();
	SocketToConnect->SendData(VerifyDataMessage.data(), VerifyDataMessage.size());
...
std::string TLSHandler::GenerateServerHello(TLS1_2::TLS1_2HelloClientStruct const& ClientHello,std::string const& SpecifiedSeessionID)
{
	...
	//server hellon ska inneh�lla vilken tls variation vi anv�nder, vilket i v�rt fall �r 1.2
	...
	//0,0x3c enda cihpersuiten vi st�djer, egentligen ska vi ju ta och v�lja ut en beroende p� etc
...
std::string TLSHandler::GetDomainResourcePath(std::string const& DomainName)
{
	...
	//TODO detta �r MEGA yikes, vi hardcodar en path som den h�r ska g� till... Borde kanske ha en socket factory eller att serversockets kr�ver mer data?
...
{
	//TODO Fixa s� det faktist ger ett session id p� ett bra s�tt
...
{
	//beror egentligen p� om vi �r en server eller client men vi f�ruts�tter att vi �r en client
	std::string ReturnValue = "";
	...
	//vi verivierar att macen st�mmer
	...
	*OutVerification = VerificationResult;
	//nu vet vi �ven att vi kan ta och �ka record�sen vi f�tt
...
{
	//tar bara 1 record, eftersom det �r om�jlgit f�r oss att veta hur m�nga som faktiskt finns, eventuellt kanske man sska skicka den uppsparade poolen?
	...
	//nu s� bara klistrat vi ihop datan och skickar tillbaka den
	//OBS! Vi kan inte h�r se till att all data vi letar efter faktiskt �r med, det f�r vi g�ra l�ngre i call hierarkin
...
std::string TLSHandler::GetNextRawProtocol(MBSockets::ConnectSocket* SocketToConnect, int MaxBytesInMemory)
{
	...
	{
		//m�ste alltid kunna avg�ra l�ngden
		NewRecordData += SocketToConnect->RecieveData(MaxBytesInMemory - NewRecordData.size());
	}
	...
	while (RecievedDataOffset < NewRecordData.size())
	{
		if (!NewRecordExtracted)
		{
			...
			if (NewRecordData.size() - 5 < CurrentRecordSize)
			{
				//socketen har slutat skicka eller n�got, nu kan vi inte f� ett f�rdigt nytt record
...
bool TLSHandler::HandleAlertMessage(MBSockets::ConnectSocket* SocketToConnect,std::string const& DecryptedAlertRecord)
{
	...
	//i detta fall vill vi printa meddelandet, annars kanske vi vill spara det i n�gon form av fel array
	...
	{
		//HandleNonFatalError();
		//eftersom vi fortfarande, om inte funktionen �ver s�ger s�, ha kopplingen skickar vi ingen data, och g�r tillbaka till att recieva data
		return(true);
	}
}
...
//{
//	//n�r vi bara ska etablera en handskakning r�cker det med att vi bara processar datan naivt, kan ju han en data som rent processar datan innan ocks�
//	std::vector<std::string> ReturnValue = std::vector<std::string>(0);
//	//processadata med typ protokoll grejer antar jag
//	while (true)
//	{
//		//max �r egentligen 2<<14+2048+5, men varf�r inte ha lite marginal
...
//		//n�r vi vet att datan vi f�tt �r krytperad vill vi innan vi g�r vidare med rseten av logiken decryptera den
//		//Ny paradigm, vi g�r igenom v�r data, kollar igenom och appendar alla hela records vi hittar, finns det kvar s� slutar vi helt enkelt
//		while (true)
//		{
...
//			{
//				//avbr�ts p� ett clean sett, reverta staten av tcp grejen
//				RecieveDataState.CurrentRecordPreviousData = "";
//				break;
//			}
//		}
//		//vi har f�tt all krypterad data, nu okrypterar vi den
//		if (ConnectionParameters.HandshakeFinished)
//		{
...
//		//Vi ska ocks� ta att och processa datan s� vi fp�r plaintext
...
//			else
//			{
//				//Unknown error, n�got har kajkat en del n�gonstans
//				assert(false);
//			}
//		}
//	}
...
{
	//d� ska vi ocks� ta skicka att vi st�nger av
...
/*Gammal logik f�r hur vi gettar protocol

		while (true)
		{
			TLS1_2::NetWorkDataHandler DataReader(reinterpret_cast<const uint8_t*> (&RawData.c_str()[OffsetOfProcessedRecords]));
			//vi vill bara komma �t l�ngden
			DataReader.Extract24();
			...
			RawDataProtocols.push_back(RawData.substr(OffsetOfProcessedRecords, 5 + LengthOfRecord));
			//vi beh�ver se till att all data kommer med, d�rf�r ser vi till att l�ngden av alla records som skickas �r l�ngden av datan, annars v�ntar vi igen p� mer data
