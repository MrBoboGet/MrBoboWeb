...
{
	//om det �r mindre bara g�r vi inget
...
void MrBigInt::NegateNumber()
{
	IsNegative = !IsNegative;
}

bool MrBigInt::IsZero() const
{
...
void MrBigInt::AddUnsigned(MrBigInt const& NumberToAdd)
{
	...
	if (Carry == 1)
	{
		//forts�tter addera tills carryn inte �r 0
...
{
	//f�ruts�tter att v�nsterledet alltid �r positivt och h�gerledet negativt
	...
	{
		//f�rst och fr�mst s� subtraherar vi med carry, �r talet UNIT_MAX s�tter vi nya carryn till 1 igen
		char NewCarry = 0;
	...
	//TODO borde man ta bort leading nollor f�r att g�ra det aningen mer minnes effektivt och ha f�rre special fall?
	Normalize();
}
...
void MrBigInt::AddWithOffset(MrBigInt& LeftInt, MrBigInt const& NumberToAdd, int IndexToStop, unsigned int Offset)
{
...
bool MrBigInt::UnsignedGreaterThan(MrBigInt const& LeftInt, MrBigInt const& RightInt, unsigned int Offset)
{
	...
	//antar att dem �r normaliserade, och att uniten till 
...
UnitType MrBigInt::ReciproalUnit32(UnitType UnitToInvert)
{
	//un
	...
	Temp = v1;
	Temp *= d31;
	eTemp -= Temp;//st�r "umullho" p� denna rad
...
{
	//inte destruktiv, s� vi tar och kopierar dividenten och denna
	...
	for (int j = m; j >= 0; j--)
	{
		...
		{
			//addback utg�r jag ifr�n bara resettar den, s� vi subtraherar bara om vi vet att den beh�va
			//MrBigInt UCopy(u);
			Temp3 = q * v;
			SubtractWithOffset(u, Temp3, j + n + 1, j);
			//asserta grejer f�r debug
...
void MrBigInt::UnsignedDivide_MB(MrBigInt const& Divident, MrBigInt& OutRemainder, MrBigInt& OutQuotient) const
{
...
void MrBigInt::UnsignedDivide(MrBigInt const& Divident, MrBigInt& OutRemainder, MrBigInt& OutQuotient) const
{
	if (Divident.InternalUnits.size() != 1)
	{
...
void MrBigInt::DoubleUnitDivide(UnitType Higher, UnitType Lower, UnitType Divisor, MrBigInt* OutQuotient, UnitType* OutRemainder, UnitType* DivisorReciprocalPointer)
{
	...
	//utifall att det inte g�r p� det normaliserade s�ttet k�r vi den andra metoden
	if (Higher >= Divisor)
	{
		UnitType NewHigher;
	...
	*OutRemainder = r.InternalUnits[0];
	...
	//TODO optimera denna kod, suger r�v, men har ingen aning om hur man ska f� end 2xUnitype/1xUnitype att ge en 2xUnitType kvot
	if (Higher >= Divisor)
	{
...
void MrBigInt::SingleLimbDivision(UnitType const* IntToDivideUnits, unsigned int NumberOfUnits, UnitType Divident, MrBigInt& OutQuotient, UnitType& OutRemainder)
{
	...
	//kopierar inten f�r att kunna normalisera den
...
void MrBigInt::KaratsubaMultiplication(MrBigInt const& LeftInt, MrBigInt const& RightInt, MrBigInt& OutResult)
{
	//bas = 2^32
...
void MrBigInt::GetHighLowUnitMultiplication(UnitType LeftUnit, UnitType RightUnit, UnitType& OutHighUnit, UnitType& OutLowUnit)
{
	OutHighUnit = 0;
	OutLowUnit = 0;
...
void MrBigInt::LongMultiplication_AOP(MrBigInt const& LeftInt, MrBigInt const& RightInt, MrBigInt& OutResult)
{
...
void MrBigInt::LongMultiplication_MB(MrBigInt const& LeftInt, MrBigInt const& RightInt, MrBigInt& OutResult)
{
	...
	OutResult = MrBigInt(0);
	//reservar s� den har plats med minst b�da
...
void MrBigInt::SetBitByIndex(size_t Index, char BitValue)
{
...
unsigned int MrBigInt::GetBitByIndex(unsigned int Index) const
{
...
{
	//funktionen antar att talet �r normaliserat och inte har leading nollor
...
void MrBigInt::PowM_SlidinWindow(MrBigInt const& Base, MrBigInt const& Exponent, MrBigInt const& Mod, MrBigInt& OutResult)
{
	//DEBUG GREJER
	...
	while (i >= 0)
	{
		...
		{
			//kalkylerar l�ngden p� bitstringen
			int L = i - K + 1;
			while ((i - 1 - L) <= K && L < i)
			{
				//L += 1;
			...
			//r�knar ut indexen i GListan som vi vill ha
...
{
	//itererar �ver exponentsens bits
...
void MrBigInt::Pow(MrBigInt const& Base, unsigned int Exponent, MrBigInt& OutResult)
{
...
void MrBigInt::SetFromBigEndianArray(const void* Data, unsigned int NumberOfBytes)
{
...
void MrBigInt::SetFromLittleEndianArray(const void* Data, unsigned int NumberOfBytes)
{
	unsigned char BytesInUnit = (UNIT_BITS / 8);
	unsigned int NumberOfElements = NumberOfBytes / BytesInUnit;
	unsigned char* CharData = (unsigned char*)Data;
...
MrBigInt& MrBigInt::operator<<=(unsigned int BitsToShift)
{
	...
	//kollar huruvida den enskilda biten p� slutet av den f�rsta kommer overflowa, d� beh�ver vi en till unit
	///*
...
MrBigInt MrBigInt::operator<<(unsigned int BitsToShift) const
{
	if (BitsToShift == 0)
	{
		return(*this);
...
std::string MrBigInt::GetBigEndianArray()
{
...
MrBigInt& MrBigInt::operator*=(const MrBigInt& RightInt)
{
...
MrBigInt& MrBigInt::operator/=(MrBigInt const& RightInt)
{
	//l�ngsam division, l�ng division
	MrBigInt Remainder;
	MrBigInt Quotient;
	MrBigInt::UnsignedDivide_AOP(*this, RightInt, Remainder, Quotient);
...
MrBigInt& MrBigInt::operator+=(const MrBigInt& RightInt)
{
	...
	{
		//TODO kan optimseras genom att specifia vilket led som �r h�ger och v�nster i funktionen
...
MrBigInt& MrBigInt::operator-=(MrBigInt const& RightInt)
{
	...
	if (IsNegative && RightInt.IsNegative)
	{
		//v�nsterlaedet negativt, h�gleredet negativt, samma sak som -> VL-HL = HL-abs(VL)
...
bool MrBigInt::operator<=(MrBigInt const& OtherInt) const
{
	if ((*this) < OtherInt)
	{
		return(true);
...
std::string MrBigInt::GetString() const
{
	std::string Temp = "";
	//clock_t TotalTimeTimer = clock();
...
std::string MrBigInt::GetHexEncodedString() const
{
	std::string ReturnValue = "";
...
std::string MrBigInt::GetBinaryString() const
{
	std::string ReturnValue = "";
...
std::string MrBigInt::GetLittleEndianString() const
{
	std::string ReturnValue = "";
